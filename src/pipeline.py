...
class DarijaDatasetPipeline:
    ...
    def __init__(self, output_file: str = None, use_scraper: bool = True):
        ...
        if use_scraper:
            try:
                ...
                logger.info(
                    "Mode sans scraper activé — utilisez process_comments_batch()"
                )
                self.scraper = None
    ...
    def process_multiple_posts(
        self, post_urls: List[str], batch_size: int = 5, save_interval: int = 10
    ) -> int:
        """
            ...
            for i, url in enumerate(post_urls, 1):
                try:
                    ...
                    if i % save_interval == 0:
                        ...
                        logger.info(
                            f"💾 Sauvegarde intermédiaire: {total_darija} Darija"
                        )
        ...
        for comment in comments:
            try:
                ...
                if self.language_detector.is_darija(cleaned_text):
                    darija_comments.append({"text": cleaned_text, "url": url})
    ...
    def process_comments_batch(self, comments: List[Dict[str, str]]) -> int:
        ...
        logger.info(
            f"✓ {len(darija_comments)}/{len(comments)} commentaires Darija "
    ...
    def process_comments_streaming(
        self, comments_iter, batch_size: int = 500, save_every: int = 2000
    ) -> int:
        """
        ...
        for comment in comments_iter:
            ...
            if len(batch) >= batch_size:
                ...
                if self.processed_count % save_every < batch_size:
                    self.save_dataset(mode="w")
                    logger.info(
                        f"💾 Sauvegarde: {total_darija} Darija / {self.processed_count} total"
                    )
    ...
    def get_statistics(self) -> Dict[str, any]:
        ...
        stats = {
            ...
            "darija_percentage": (
                self.darija_count / max(self.processed_count, 1) * 100
            ),
    ...
    def print_statistics(self):
        ...
        print(
            f"  Pourcentage Darija:            {stats.get('darija_percentage', 0):.2f}%"
        )
        print(f"  ---")
        print(f"  Enregistrements CSV:           {stats.get('total_records', 0):,}")
        print(f"  URLs uniques:                  {stats.get('unique_urls', 0):,}")
        print(
            f"  Longueur moyenne texte:        {stats.get('avg_text_length', 0):.1f} chars"
        )
        print(f"  ---")
        print(f"  Temps écoulé:                  {elapsed:.1f}s")
        print(
            f"  Vitesse:                       {stats.get('comments_per_second', 0):.0f} comments/s"
        )
        print("=" * 60 + "\n")
