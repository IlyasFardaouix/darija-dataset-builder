...
class CSVManager:
    ...
    def add_record(self, text: str, url: str):
        ...
        """
        if text and url:
            self.data.append({"text": text.strip(), "url": url.strip()})
    ...
    def save_to_csv(self, output_file: str = None, mode: str = "w") -> str:
        ...
        try:
            ...
            df.to_csv(
                output_file, index=CSV_CONFIG["index"], encoding=CSV_CONFIG["encoding"]
            )
            logger.info(
                f"Données sauvegardées: {len(df)} enregistrements dans {output_file}"
            )
            return str(output_file)
